...
- Table I: Cross-Architecture baselines
- Table II: Tier-wise SIPR, MMD^2, GCR
- Table III: N=20 TOST equivalence & downstream Macro-F1
- Section IV stats: Spearman rho, triage effect, ResNet50d replication
- Artifact inspection: Confirms all 4 figures exist and are non-empty
"""
import os
import json
import glob
...
def run_claims_audit():
    ...
    def check(name, claimed, observed, tol=1e-3, is_str=False):
        ...
        print(f" {status_str} {name:<48} | Claimed: {str(claimed):<12} | Observed: {str(observed)}")

    # 1. Check Table II
    print("\n--- AUDITING TABLE II (Tier-wise SIPR, MMD^2, GCR) ---")
    ...
    # 4. Check Section IV-A / IV-C Core Stats
    ...
    # 5. Check Paper Figures
    print("\n--- AUDITING RENDERED FIGURE ARTIFACTS ---")
    figures = [
        "fig1_longtail.png",
        "fig2_spectrogram_grid.png",
        "fig_baseline_spectrograms.png",
        "fig_degradation_curve_n20.png"
    ]
    for fig_name in figures:
        f_path = os.path.join(FIGURES_DIR, fig_name)
        exists = os.path.exists(f_path) and os.path.getsize(f_path) > 1000
